...
public class NavigationHandler2009 implements RobotDeviceListener {
	...
	private BlockingQueue<RobotDeviceRequest> queue = new ArrayBlockingQueue<RobotDeviceRequest>(5, true);
	...
	public List<RobotDeviceRequest> getRequests(int configurationId, int trajectoryIndex, String suffix, Holder<Double> initialOrientation) {
	...
	private void handleCommand(List<RobotDeviceRequest> requests, String command) {
		if (command.equals(TAKE_COLUMN)) {
			...
			RobotDeviceRequest request = new Sequence2009Request(Sequence2009Request.Action.PREPARE_TO_BUILD_FIRST_CONSTRUCTION);
			requests.add(request);
			...
			RobotDeviceRequest request = new Sequence2009Request(Sequence2009Request.Action.FIRST_BUILD_COLUMNS_AND_LINTEL_TYPE_2);
			requests.add(request);
			// Phase 2 / Type 1
		} else if (command.equals(PREPARE_TO_BUILD_SECOND_CONSTRUCTION)) {
			RobotDeviceRequest request = new Sequence2009Request(Sequence2009Request.Action.PREPARE_TO_BUILD_SECOND_CONSTRUCTION);
			requests.add(request);
			...
			RobotDeviceRequest request = new Sequence2009Request(Sequence2009Request.Action.PREPARE_TO_LOAD_SECOND_LINTEL);
			requests.add(request);
